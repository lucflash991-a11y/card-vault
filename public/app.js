...
const FUNKO_LOCAL_KEY="cardvault.funko.v302.items";let funkoItems=[],funkoCloudUnsub=null,funkoStream=null,funkoScanTimer=null,funkoHtml5Scanner=null;
...
function updateFunkoBarcodeSupport(){if(!$("funkoBarcodeSupport"))return;$("funkoBarcodeSupport").textContent=("BarcodeDetector" in window||typeof Html5Qrcode!=="undefined")?"Camera barcode scanner ready.":"Camera scanner unavailable — enter the UPC manually below."}
async function startFunkoScanner(){
if(funkoStream||funkoHtml5Scanner)return;
if("BarcodeDetector" in window){try{const formats=await BarcodeDetector.getSupportedFormats(),wanted=["upc_a","upc_e","ean_13","ean_8"].filter(x=>formats.includes(x)),detector=new BarcodeDetector({formats:wanted.length?wanted:undefined});funkoStream=await navigator.mediaDevices.getUserMedia({video:{facingMode:{ideal:"environment"}},audio:false});const v=$("funkoBarcodeVideo");v.srcObject=funkoStream;await v.play();v.closest(".funko-scanner-card").classList.add("scanning");$("funkoStartScanBtn").classList.add("hidden");$("funkoStopScanBtn").classList.remove("hidden");const tick=async()=>{if(!funkoStream)return;try{const codes=await detector.detect(v),hit=codes.find(c=>/^\d{8,14}$/.test(c.rawValue||""));if(hit){const code=hit.rawValue;await stopFunkoScanner();$("funkoBarcodeInput").value=code;await lookupFunkoBarcode(code);return}}catch{}funkoScanTimer=setTimeout(tick,350)};tick();return}catch(err){console.warn("Native scanner failed",err);await stopFunkoScanner()}}
if(typeof Html5Qrcode!=="undefined"){try{const formats=[Html5QrcodeSupportedFormats.UPC_A,Html5QrcodeSupportedFormats.UPC_E,Html5QrcodeSupportedFormats.EAN_13,Html5QrcodeSupportedFormats.EAN_8];funkoHtml5Scanner=new Html5Qrcode("funkoHtml5Reader",{formatsToSupport:formats,verbose:false});const card=$("funkoHtml5Reader").closest(".funko-scanner-card");$("funkoHtml5Reader").classList.remove("hidden");card.classList.add("html5-scanning");$("funkoStartScanBtn").classList.add("hidden");$("funkoStopScanBtn").classList.remove("hidden");$("funkoBarcodeSupport").textContent="Point the camera at the barcode and hold steady.";await funkoHtml5Scanner.start({facingMode:"environment"},{fps:10,qrbox:{width:280,height:140}},async text=>{const code=String(text||"").replace(/\D/g,"");if(!/^\d{8,14}$/.test(code))return;$("funkoBarcodeInput").value=code;await stopFunkoScanner();await lookupFunkoBarcode(code)},()=>{});return}catch(err){console.warn("Safari scanner failed",err);await stopFunkoScanner();$("funkoBarcodeSupport").textContent="Could not start camera. Check Safari camera permission.";toast("Could not start camera scanner");return}}
toast("Camera scanner unavailable — enter UPC manually")
}
async function stopFunkoScanner(){if(funkoScanTimer){clearTimeout(funkoScanTimer);funkoScanTimer=null}if(funkoStream){funkoStream.getTracks().forEach(t=>t.stop());funkoStream=null}if(funkoHtml5Scanner){try{if(funkoHtml5Scanner.isScanning)await funkoHtml5Scanner.stop()}catch{}try{await funkoHtml5Scanner.clear()}catch{}funkoHtml5Scanner=null}const card=$("funkoBarcodeVideo")?.closest(".funko-scanner-card");card?.classList.remove("scanning","html5-scanning");$("funkoHtml5Reader")?.classList.add("hidden");$("funkoStartScanBtn")?.classList.remove("hidden");$("funkoStopScanBtn")?.classList.add("hidden");updateFunkoBarcodeSupport()}
...
async function lookupFunkoBarcode(code){code=String(code||"").replace(/\D/g,"");if(code.length<8){toast("Enter a valid UPC/EAN");return}$("funkoLookupStatus").textContent="Looking up barcode…";$("funkoResults").innerHTML="";try{const r=await fetch(`/api/funko/barcode/${encodeURIComponent(code)}`),d=await r.json();if(!r.ok)throw new Error(d.error||"Not found");const rows=(Array.isArray(d.products)&&d.products.length?d.products:(d.product?[d.product]:[])).map(normalizeFunko);$("funkoLookupStatus").textContent=rows.length?"Product found • 0 AI calls":"Barcode wasn't found. Try name search below.";renderFunkoResults(rows)}catch(err){console.warn(err);$("funkoLookupStatus").textContent=String(err?.message||"Barcode lookup failed. Try again or use name search.");renderFunkoResults([])}}
...
const COMIC_LOCAL_KEY="cardvault.comics.v333.items";
...
function normalizeComic(x){
  ...
  return {
    ...
    gcdId:String(x.gcdId||""),gcdUrl:String(x.gcdUrl||""),metronId:String(x.metronId||""),
    source:String(x.source||x.candidateSource||"GCD"),matchScore:Number(x.matchScore||0),matchReason:String(x.matchReason||""),
    userCoverImage:String(x.userCoverImage||""),
...
function comicCoverFallback(img,c){
  if(!img)return;
...
function comicMarkup(c){return `<button class="comic-item" data-comic-item="${esc(c.id)}" type="button"><img src="${esc(comicCoverSrc(c))}" alt="" onerror="this.onerror=null;this.src='${esc(c.userCoverImage||'/icons/card-placeholder.svg')}'"><strong>${esc(c.title)}</strong><span>${c.issueNumber?`#${esc(c.issueNumber)} • `:""}${esc(c.publisher||"Comic")}</span><span>${comicMoney(c.value)}</span></button>`}
...
async function searchComics(title,issue,publisher){
  ...
  try{
    ...
    const rows=(d.items||[]).map(normalizeComic);
    $("comicLookupStatus").textContent=rows.length?`${rows.length} comic match${rows.length===1?"":"es"} • choose the exact cover`:"No GCD matches. Check the title/issue or try a cover photo.";
    renderComicResults(rows);
