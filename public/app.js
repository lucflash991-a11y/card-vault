...
function openOfferDetail(id){const o=socialOffers.find(x=>x.id===id);if(!o)return;const mine=o.fromUid===currentUser?.uid,otherUid=mine?o.toUid:o.fromUid,other=publicProfiles.find(p=>p.uid===otherUid),give=offeredSideValue(o),get=targetSideValue(o),warn=Math.max(give,get)>0&&Math.abs(give-get)/Math.max(give,get)>.35;$("offerDetailTitle").textContent=`${String(o.status||"pending").toUpperCase()} OFFER`;$("offerDetailBody").innerHTML=`<div class="offer-detail-card"><img src="${o.targetCard?.front||"/icons/card-placeholder.svg"}"><div><h3>${esc(o.targetCard?.player||"Card")}</h3><p>${mine?"To":"From"} @${esc(other?.username||"collector")}</p><p class="offer-expiry">${offerExpiryText(o)}</p></div></div><div class="offer-value-balance"><strong>Estimated values</strong><span>Offered side: ${money(give)}</span><span>Target card: ${money(get)}</span>${warn?'<span class="offer-value-warning">Values are pretty far apart.</span>':""}</div>${o.note?`<div class="offer-value-balance"><strong>Note</strong><span>${esc(o.note)}</span></div>`:""}`;const a=$("offerDetailActions");a.innerHTML="";if(o.status==="pending"&&o.toUid===currentUser.uid&&!offerExpired(o))a.innerHTML='<button class="accept" data-os="accepted">Accept</button><button class="decline" data-os="declined">Decline</button><button data-counter>Counter</button><button data-msg>Message</button>';else if(o.status==="pending"&&o.fromUid===currentUser.uid)a.innerHTML='<button class="cancel" data-os="cancelled">Cancel Offer</button><button data-msg>Message</button>';else if(o.status==="accepted")a.innerHTML='<button class="accept" data-os="completed">Mark Completed</button><button data-msg>Message</button>';else a.innerHTML='<button data-msg>Message</button>';a.querySelectorAll("[data-os]").forEach(b=>b.onclick=()=>updateOfferStatus24(o,b.dataset.os));a.querySelector("[data-counter]")?.addEventListener("click",()=>openCounterOffer(o));a.querySelector("[data-msg]")?.addEventListener("click",()=>openChatWith(otherUid));$("offerDetailModal").classList.remove("hidden");document.body.classList.add("modal-open")}
...
async function submitCounter(){const o=socialOffers.find(x=>x.id===activeCounterOfferId);if(!o)return;const amt=Number($("counterCashAmount").value||0),note=$("counterNote").value.trim();await firebase.setDoc(firebase.doc(firebase.db,"publicOffers",o.id),{cashAmount:amt,note:note||o.note,status:"pending",counteredBy:currentUser.uid,updatedAt:Date.now(),expiresAt:Date.now()+172800000},{merge:true});const target=o.fromUid===currentUser.uid?o.toUid:o.fromUid;await notifyUser(target,"counter",`Counter offer on ${o.targetCard?.player||"card"}`,{offerId:o.id});closeCounterOffer();closeOfferDetail();toast("Counter sent")}
async function updateOfferStatus24(o,status){const payload={status,updatedAt:Date.now()};if(status==="completed")payload.completedAt=Date.now();await firebase.setDoc(firebase.doc(firebase.db,"publicOffers",o.id),payload,{merge:true});const target=o.fromUid===currentUser.uid?o.toUid:o.fromUid;await notifyUser(target,status,`${status}: ${o.targetCard?.player||"card"}`,{offerId:o.id});closeOfferDetail();toast(status==="cancelled"?"Offer cancelled":status)}
