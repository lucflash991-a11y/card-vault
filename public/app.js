...
async function openChatWith(uid){if(hasBlocked(uid)||isBlockedBy(uid)){toast("Messaging unavailable");return}activeChatOtherUid=uid;activeConversationId=await ensureConversation(uid);const p=publicProfiles.find(x=>x.uid===uid);$("chatName").textContent=p?.displayName||"Collector";$("chatHandle").textContent=`@${p?.username||"collector"}`;renderChat();$("chatModal").classList.remove("hidden");document.body.classList.add("modal-open");$("chatInput")?.focus()}
function closeChat(){$("chatModal").classList.add("hidden");document.body.classList.remove("modal-open");activeConversationId=null;activeChatOtherUid=null}
...
async function sendChat(){const input=$("chatInput"),text=input?.value.trim();if(!text||!activeConversationId||!activeChatOtherUid||!currentUser||!firebase?.db)return;const btn=$("chatSendBtn");if(btn)btn.disabled=true;try{await firebase.setDoc(firebase.doc(firebase.collection(firebase.db,"publicMessages")),{conversationId:activeConversationId,senderUid:currentUser.uid,text,createdAt:Date.now()});await firebase.setDoc(firebase.doc(firebase.db,"publicConversations",activeConversationId),{participants:[currentUser.uid,activeChatOtherUid],lastMessage:text,updatedAt:Date.now()},{merge:true});input.value="";await notifyUser(activeChatOtherUid,"message","New message",{conversationId:activeConversationId});}catch(e){console.warn("Message send failed:",e);toast("Could not send message");}finally{if(btn)btn.disabled=false;input?.focus()}}
